...
start(Name, Transport, Port, Services, Options) ->
    ...
    %% request.
    Dispatch = cowboy_router:compile([{'_', [{"/:service/:method",
                                              ?MODULE,
                                              #{auth_fun => AuthFun,
                                                services => Services}}]}]),
...
read_frames(#{cowboy_req := Req, encoding := Encoding} = Stream) ->
...
respond_and_continue(T, Response, NewStream, NewState, More) ->
    try
        grpc:send(NewStream#{handler_state => NewState}, Response)
    of
        SentStream ->
...
execute(Msg, #{handler := Module, service := Service, rpc := Function,
               decoder := Decoder, handler_state := State} = Stream) ->
    try
        grpc_lib:decode_input(Service, Function, Decoder, Msg)
    of
        Decoded ->
            try
...
finalize(#{cowboy_req := Req, trailers := Trailers}, Status, <<"">>) ->
    _R =
        cowboy_req:stream_trailers(Trailers#{<<"grpc-status">> => Status}, Req),
    {ok, Req, []};
finalize(#{trailers := Trailers} = Stream, Status, Message) ->
    finalize(Stream#{trailers => Trailers#{<<"grpc-message">> => Message}},
             Status,
             <<"">>).
...
split_frame(<<0, Length:32, Encoded:Length/binary, Rest/binary>>,
            Encoding, Acc) ->
