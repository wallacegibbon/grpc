...
-spec start(Name :: term(), Transport :: tcp | ssl,
            Port :: integer(), Services :: grpc:services(),
            Options :: [grpc:server_option()]) -> {ok,
...
start(Name, Transport, Port, Services, Options) ->
    ...
    ProtocolOpts = #{env => #{dispatch => Dispatch},
                     ...
                     stream_handlers =>
                         [grpc_stream_handler, cowboy_stream_h],
    ...
    case Transport of
        ...
        ssl ->
            TransportOpts = [{port, Port}
                             | proplists:get_value(transport_options,
                                                   Options,
                                                   [])],
...
stop(Name) -> cowboy:stop_listener(Name).
...
init(Req, Options) ->
    ...
    case authenticate(Req, Options) of
        ...
        {true, ClientInfo} ->
            try authenticated(Stream#{client_info => ClientInfo},
                              Options)
            catch
                {Code, Reason} -> finalize(Stream, Code, Reason)
            end
    end.

make_stream(#{headers := Headers, host := Authority,
              scheme := Scheme, path := Path, method := Method} =
                Req) ->
    maps:fold(fun process_header/3,
              #{cowboy_req => Req, authority => Authority,
                scheme => Scheme, path => Path, method => Method,
                headers => #{}, trailers => #{},
                ...
                encoding => plain,
                compression =>
                    none, %% compression of the response messages
                start_time => erlang:system_time(1),
                content_type => undefined, user_agent => undefined,
                timeout => infinity},
              Headers).
...
process_header(<<"te">>, _Value, Acc) -> Acc;
...
process_header(Key, Value,
               #{metadata := Metadata} = Acc) ->
    {_, DecodedValue} = grpc_lib:maybe_decode_header({Key,
                                                      Value}),
    ...
    case proplists:get_value(auth_fun, Options) of
        undefined ->
            case proplists:get_value(client_cert_dir, Options) of
                undefined -> undefined;
                Dir -> grpc_lib:auth_fun(Dir)
            end;
        Fun -> Fun
    end;
get_authfun(_, _) -> undefined.
...
authenticate(Req, #{auth_fun := AuthFun})
    when is_function(AuthFun) ->
    case cowboy_req:cert(Req) of
        undefined -> false;
        Cert when is_binary(Cert) -> AuthFun(Cert)
    end;
authenticate(_Req, _Options) -> {true, undefined}.

authenticated(#{cowboy_req := Req} = Stream, Options) ->
    %% invoke the rpc (= function) for the service (= module).
    try get_function(Req, Options, Stream) of
        NewStream -> read_frames(NewStream)
    catch
        _:_ ->
...
get_function(Req, #{services := Services} = _Options,
             Stream) ->
    QualifiedService = cowboy_req:binding(service, Req),
    Service =
        binary_to_existing_atom(lists:last(binary:split(QualifiedService,
                                                        <<".">>,
                                                        [global]))),
    ...
    DecoderModule = maps:get(decoder,
                             Spec,
                             Handler:decoder()),
    {module, _} = code:ensure_loaded(DecoderModule),
    Rpc = binary_to_existing_atom(cowboy_req:binding(method,
                                                     Req)),
    Stream#{decoder => DecoderModule, service => Service,
            handler => Handler, handler_state => HandlerState,
            rpc => Rpc}.

read_frames(#{cowboy_req := Req, encoding := Encoding} =
                Stream) ->
...
process_messages([Message | T], Stream, More) ->
    case execute(Message, Stream) of
        ...
        {error, Code, ErrorMessage, NewStream}
            when is_integer(Code), is_binary(ErrorMessage) ->
            finalize(NewStream,
        ...
        _Other ->
            finalize(Stream,
                     ...
                     <<"Internal error - unexpected response "
                       "value">>)
    end;
process_messages([], Stream, More) ->
    case More of
        ok -> respond_and_finalize(execute(eof, Stream));
        more -> read_frames(Stream)
    end.

respond_and_finalize({Response, NewStream}) ->
    try grpc:send(NewStream, Response) of
        SentStream -> finalize(SentStream)
    catch
        {error, Status, Message} ->
            finalize(NewStream, Status, Message);
        {error, Status, Message, CurrentStream} ->
            finalize(CurrentStream, Status, Message)
    end.

respond_and_continue(T, continue, NewStream, NewState,
                     More) ->
    process_messages(T,
                     NewStream#{handler_state => NewState},
                     More);
respond_and_continue(T, Response, NewStream, NewState,
                     More) ->
    try grpc:send(NewStream#{handler_state => NewState},
                  Response)
    of
        SentStream -> process_messages(T, SentStream, More)
    catch
        {error, _, Message} ->
...
execute(Msg,
        #{handler := Module, service := Service,
          rpc := Function, decoder := Decoder,
          handler_state := State} =
            Stream) ->
    try grpc_lib:decode_input(Service,
                              Function,
                              Decoder,
                              Msg)
    of
        ...
        _:_ ->
            {error,
             ?GRPC_STATUS_INTERNAL_INT,
             <<"Error parsing request protocol buffer">>,
             Stream}
    end.
...
finalize(#{headers_sent := false} = Stream, Status,
         Message) ->
...
finalize(#{cowboy_req := Req, trailers := Trailers},
         Status, <<"">>) ->
    _R =
        cowboy_req:stream_trailers(Trailers#{<<"grpc-status">>
                                                 => Status},
                                   Req),
    {ok, Req, []};
finalize(#{trailers := Trailers} = Stream, Status,
         Message) ->
...
split_frame(<<>>, _Encoding, Acc) -> lists:reverse(Acc);
split_frame(<<0, Length:32, Encoded:Length/binary,
              Rest/binary>>,
            Encoding, Acc) ->
    split_frame(Rest, Encoding, [Encoded | Acc]);
split_frame(<<1, Length:32, Compressed:Length/binary,
              Rest/binary>>,
            Encoding, Acc) ->
    Encoded = case Encoding of
                  <<"gzip">> -> zlib:gunzip(Compressed);
                  _ ->
                      throw({?GRPC_STATUS_UNIMPLEMENTED,
