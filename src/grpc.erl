...
compile(FileName) -> grpc:compile(FileName, []).

-spec compile(FileName :: string(),
              Options :: gbp_compile:opts()) -> ok.
...
send_headers(#{cowboy_req := Req, headers := Metadata,
               compression := Compression, headers_sent := false} =
                 Stream,
             Headers) ->
    MergedHeaders =
