...
start_grpc_server() ->
    {ok, _} = grpc:start_server(load_test,
                                tcp,
                                10000,
                                #{get_stats =>
                                      #{handler => statistics_server}}),
    log_msg("Server started~n", []).

start_native_server() -> ok.
...
start_client() ->
    ...
    Server = case Rpc_type of
                 ...
                 grpc -> ServerArg;
                 undefined ->
                     io:format("Error: server argument must be provided~n")
             end,
    start(Server, N_workers, N_reqs, Rpc_type).

get_arg_val(Tag, Def_val) ->
    try [Val_str] = proplists:get_value(Tag,
                                        init:get_arguments()),
        case Tag of
            server -> Val_str;
            rpc_type ->
...
start(Server, Num_workers, Num_reqs, Rpc_type) ->
    ...
    ets:new(worker_pids,
            [named_table,
             ...
             {write_concurrency, true},
             {read_concurrency, true}]),
    log_msg("Starting spawn of ~p workers...~n",
            [Num_workers]),
...
worker_init(Num_reqs, Rpc_type = grpc, Server)
    when is_list(Server) ->
    receive
        start ->
...
update_ts_counter(Key, Val) ->
    ...
    ets:update_counter(ts_counters,
                       Key_ts,
                       {2, Val},
                       {Key_ts, 0}).
...
log_msg(Fmt, Args) ->
    io:format("~s -- " ++ Fmt,
              [printable_ts(calendar:local_time()) | Args]).

print_results() ->
    Dict = lists:foldl(fun ({{Ts, Key}, Val}, X_dict) ->
                               dict:update(Ts,
                                           fun (Res_tvl) ->
                                                   [{Key, Val} | Res_tvl]
                                           end,
    ...
    io:format("~n", []),
    io:format("Start time                   : ~s~n",
              [printable_ts(Start_time)]),
    io:format("End time                     : ~s~n",
              [printable_ts(End_time)]),
    io:format("Elapsed time (seconds)       : ~p~n",
              [TT_secs]),
    io:format("Number of attempted requests : ~p~n",
              [Num_reqs]),
    io:format("Req/sec                      : ~p~n",
              [ReqsPerSec]),
    io:format("~n", []),
    io:format("~-30.30. s success, timeout, not_connected~n",
              ["Timestamp"]),
    lists:foreach(fun ({X_key, X_vals}) ->
                          X_succ = proplists:get_value(success, X_vals, 0),
                          X_timeout = proplists:get_value(timeout, X_vals, 0),
                          X_not_conn = proplists:get_value(not_connected,
                                                           X_vals,
                                                           0),
                          io:format("~s\t\t~10.10. w, ~7.7. w, ~13.13. w~n",
                                    [printable_ts(X_key),
                                     X_succ,
                                     X_timeout,
                                     X_not_conn])
...
printable_ts({{Y, M, D}, {H, Mi, S}}) ->
    io_lib:format("~p-~2.2.0w-~2.2.0w_~2.2.0w:~2.2.0w:~2.2.0w",
                  [Y, M, D, H, Mi, S]).
