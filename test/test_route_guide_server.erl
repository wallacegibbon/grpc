...
-type 'RouteSummary'() :: #{point_count => integer(),
                            feature_count => integer(), distance => integer(),
                            elapsed_time => integer()}.

-type 'Point'() :: #{latitude => integer(),
                     longitude => integer()}.

-type 'Rectangle'() :: #{lo => 'Point'(),
                         hi => 'Point'()}.

-type 'RouteNote'() :: #{location => 'Point'(),
                         message => string()}.
...
-spec 'GetFeature'(Message :: 'Point'(),
                   Stream :: grpc:stream(),
                   State :: any()) -> {'Feature'(), grpc:stream()} |
                                      grpc:error_response().
...
'GetFeature'(Message, Stream, _State) ->
    ...
    case grpc:metadata(Stream) of
        #{<<"password">> := <<"secret">>} -> {Feature, Stream};
        #{<<"password">> := _} ->
...
-spec 'ListFeatures'(Message :: 'Rectangle'(),
                     Stream :: grpc:stream(),
                     State :: any()) -> {['Feature'()], grpc:stream()} |
                                        grpc:error_response().
...
'ListFeatures'(_Message, Stream, _State) ->
    ...
    Stream3 = grpc:set_trailers(Stream2,
                                #{<<"nr_of_points_sent">> => <<"2">>}),
    {[#{name => "Tour Eiffel",
        location => #{latitude => 3, longitude => 5}}],
     Stream3}.

-spec 'RecordRoute'(Message :: 'Point'() | eof,
                    Stream :: grpc:stream(), State :: any()) -> {continue,
                                                                 gprc:stream(),
                                                                 any()} |
                                                                {'RouteSummary'(),
                                                                 grpc:stream()}.
...
'RecordRoute'(FirstPoint, Stream, undefined) ->
    ...
    {continue,
     Stream,
     #{t_start => erlang:system_time(1),
       acc => [FirstPoint]}};
'RecordRoute'(eof, Stream,
              #{t_start := T0, acc := Points}) ->
...
-spec 'RouteChat'(Message :: 'RouteNote'() | eof,
                  Stream :: grpc:stream(),
                  State :: any()) -> {['RouteNote'()], gprc:stream(),
                                      any()} |
...
'RouteChat'(In, Stream, undefined) ->
    'RouteChat'(In, Stream, []);
'RouteChat'(eof, Stream, _) -> {[], Stream};
'RouteChat'(#{location := Location} = P, Stream,
            Data) ->
