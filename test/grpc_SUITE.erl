...
end_per_group(Group, _Config)
        when Group == tutorial; Group == metadata; Group == more_services;
             Group == h2_client; Group == security; Group == compressed;
             Group == authenticated; Group == security_issues;
             Group == middlewares ->
...
run_bonjour(Config) ->
    ...
    {ok, #{result := #{message := "Bonjour, World"}}} =
        grpc_client:unary(Connection, #{name => "World"},
                          'Greeter', 'SayHello', helloworld, []).
...
run_listfeatures(Config) ->
    ...
    {data,
     #{location := #{latitude := 4, longitude := 5}, name := "Louvre"}} =
...
send_big_message(Stream, Message, N) ->
    ...
    ok = grpc_client:send(Stream, #{location => P, message => Message}),
